...
fn run_echo_tool() {
    ...
    // 5. Call execute::execute_tool
    let (exit_code, _outputs) =
        execute::execute_tool(&tool, &inputs, &outdir, &runtime, &log_dir, "echo")
            .expect("execute_tool failed");

    // 6. Assert exit_code == 0
    assert_eq!(exit_code, 0, "echo tool should exit with code 0");

    // 7. Read logs/echo.stdout.log and assert it contains "hello-zen"
